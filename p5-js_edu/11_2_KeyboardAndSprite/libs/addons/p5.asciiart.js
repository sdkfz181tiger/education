...
/*
  ...
  The class constructor can be called with none, one, two or three parameters:
    new AsciiArt();
    new AsciiArt(_fontName);
    new AsciiArt(_fontName, _fontSize);
    new AsciiArt(_fontName, _fontSize, _textStyle);
*/
p5.prototype.AsciiArt = function(_fontName, _fontSize, _textStyle) {
  /*
    "Private" variables storing information about the name, size and style of
    the font used to create the table containing glyphs sorted by size (by
    default characters occupying a larger area will be assigned to lighter
    pixels).
  */
  ...
  */
  if(arguments.length > 0) this.__fontName = _fontName;
  ...
  if(arguments.length > 2) this.__textStyle = _textStyle;
  /*
...
p5.prototype.AsciiArt.prototype.printWeightTable = function() {
  for(var i = 0; i < this.__weightTable.length; i++)
    console.log(
      '[' + i + '] ' + char(this.__weightTable[i].code) + ' ' +
...
p5.prototype.AsciiArt.prototype.createWeightTable = function() {
  ...
  this.__graphics.textAlign(CENTER, CENTER);
  ...
  for(var i = this.__range.min; i <= this.__range.max; i++) {
    ...
    this.__graphics.text(char(i), temp_w * 0.5, temp_h * 0.5);
...
p5.prototype.AsciiArt.prototype.__convert = function() {
  ...
  for(var temp_y = 0; temp_y < this.__graphics.height; temp_y++) {
    for(var temp_x = 0; temp_x < this.__graphics.width; temp_x++) {
      ...
      temp_result[temp_x][temp_y] = char(this.__weightTable[temp_weight].code);
    }  
  }
