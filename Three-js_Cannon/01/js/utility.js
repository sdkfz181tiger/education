...
class CannonManager{
	...
	constructor(pcX=0, pcY=0, pcZ=0){
		...
		// Material(Plane)
		let groundMat = new CANNON.Material("groundMat");
		let groundPhy = new CANNON.Body({mass: 0, material: groundMat});
		groundPhy.addShape(new CANNON.Plane());
		groundPhy.quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), -Math.PI / 2);
		this._world.add(groundPhy);
		...
		// Material(Sphere)
		let sphereMat = new CANNON.Material("sphereMat");
		let spherePhy = new CANNON.Body({mass: 1, material: sphereMat});
		spherePhy.addShape(new CANNON.Sphere(1));
		spherePhy.position.set(0, 10, 0);
		spherePhy.angularVelocity.set(5, 5, 5);
		spherePhy.angularDamping = 0.1;
		this._world.add(spherePhy);

		// Objects
		this._objs = [
			{mesh: groundMesh, mat: groundMat, phy: groundPhy},
			{mesh: sphereMesh, mat: sphereMat, phy: spherePhy}
		];
	...
	update(){
		...
		for(let obj of this._objs){
			obj.mesh.position.copy(obj.phy.position);
			obj.mesh.quaternion.copy(obj.phy.quaternion);
		}
		// Debugger
