...
//     2020/11/23_パズル機能追加
//     2020/12/04_トグルレイヤーの重なり順を調整
//     2020/12/18_音符を再生後に画面から消す
...
function createMboxNote(x, y, pngOff, pngOn, interval, time, wait){
	...
	mboxNotes.push({x:x, y:y, pngOff:pngOff, pngOn:pngOn, interval:interval, time:time, wait:wait});// Push
}
...
function playMbox(){
	...
	for(var i=0; i<mboxNotes.length; i++){
		...
		blinkMboxImg(mboxNote, note.pngOff, note.pngOn, note.interval, note.time, note.wait);// Blink
	}
}
...
function blinkMboxImg(target, pngOff, pngOn, interval, time, wait){
	// Timeout
	setTimeout(function(){
		target.attr({src: ICON_DIR+pngOn});// On
		setTimeout(function(){
			target.attr({src: ICON_DIR+pngOff});// Off
			if(wait != 0){// Remove_20201218
				setTimeout(function(){
					target.remove();
				}, wait);
			}
