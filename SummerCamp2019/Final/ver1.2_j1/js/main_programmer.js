...
function setStats(){
	...
	//showFireworks(0, 5, 0, 20, 40);

	// setInterval(()=>{
	// 			showFireworks(0, 5, 5, 40, 40);
	// 		}, 1000);
...
function onSeek(){
	...
	//[発表会用] 微調整
	if ((house_gole != null) && (house_gole.rotation.y != 0)) {
		console.log("onSeek check !! check !!!!");

		house_gole.rotation.set(0, 0, 0);
...
function onEnd(){
	...
	// サウンド再生
	soundLoader.playSound("cheer_j1.mp3", 1.0);// Sound
	...
	if (house_gole != null) {
		...
		let tl1 = createTimeline(0, false, ()=>{
			...
			tl.to(cam.position, 2.0, {delay: 0.0, x: "+=0", y: "+=70", z: "+=70"});// 相対位置
		
			let x = getRandom(-3, +3);
			let y = getRandom(-3, +3);
			showFireworks(x, 68 + y, 68, 40, 40);
		...
		tl1.to(house_gole.rotation, 1.0, {delay: 2.0, x: "+=0", y: "-="+(3.14/2), z: "+=0"});
		...
		//待機
		tl1.to(house_gole.rotation, 2.5, {delay: 0.0});
	}
}
