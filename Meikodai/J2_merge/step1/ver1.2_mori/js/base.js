...
function readyThreeJS(){
	...
	function checkLoaders(){
		loaderCounter--;
		if(loaderCounter <= 0){readyNotes();titleStart();}
	}
	...
	function setGUI(cTime, tTime){
		...
		let GuiCtl = function(){
			...
			this.reset  = ()=>{titleStart();};//   gameStart!!!!!!!!!!
			this.seek  = 0;
		};
		let gui    = new dat.GUI();
		gui.close();
	...
	titleStart = function(){
		...
		onkeydown = function(){
			...
			var INTERVAL = setInterval(function(){title.position.y += 0.1;if(title.position.y >= 100){clearInterval(INTERVAL);rootGroup.remove(title)}}, 1);
			onkeydown = null;
		}
	}
