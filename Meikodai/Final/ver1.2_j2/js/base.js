...
function readyThreeJS(){
	...
	function putSensors(note){
		...
		// Sensor
		let sensor = new Sensor(note.x, note.y-6, 0, note.sensor);
		sensors.push(sensor);
	}
	...
	function putMarkers(note){
		...
		for(let i=0; i<note.z.length; i++){
			...
			let shadow = objLoader.findModels("cross.obj", 0.5);
			shadow.position.set(note.x, note.y-4, 
				i*TIME_TO_PIXEL*TIME_TO_SPAN*-1.0 + OFFSET_Z_PIXEL);
			noteGroup.add(shadow);
		}
	}
}
