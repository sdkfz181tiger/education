...
enchant.Event = enchant.Class.create({
    ...
    _initPosition: function(pageX, pageY) {
        ...
        // 2017/01/23 enchant-stageの位置が変わっていてもタッチ位置に補正をかける
        // 2017/01/24 画面のスクロールにも対応
        var stage = document.getElementById('enchant-stage');
        if(stage != null){
            var scrollX = window.pageXOffset;
            var scrollY = window.pageYOffset;
            core._pageX = stage.getBoundingClientRect().left + scrollX;
            core._pageY = stage.getBoundingClientRect().top + scrollY;
        }
...
(function() {
    ...
    enchant.Core = enchant.Class.create(enchant.EventTarget, {
        ...
        initialize: function(width, height) {
            ...
            //if (initial) {// Default
            if (true) {// CodeEditor_20150801_Ozaki
...
enchant.Timeline = enchant.Class.create(enchant.EventTarget, {
 ...
     * You can choose time based and frame based(default) animation.
     ...
     * @return {enchant.Timeline}
     */
    scaleTo: function(scaleX, scaleY, time, easing) {
        return this.tween({
            scaleX: scaleX,
            scaleY: scaleY,
            time: time,
     ...
     * @return {enchant.Timeline}
     */
    scaleBy: function(scaleX, scaleY, time, easing) {
        return this.tween({
            scaleX: function() {
