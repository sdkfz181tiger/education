//==========
// Three.js
// -> https://threejs.org/

console.log("Hello Three.js!!");

window.onload = function(){
	console.log("onload!!");
	startXR();
}

function startXR(){
	var clock = new THREE.Clock();

	var container;
	var renderer, camera, scene;

	var reticle, pin;

	var ambientLight, directionalLight;
	var initialAmbientLightIntensity = 1;
	var initialDirectionalLightIntensity = 0.5;

	// ar, magicWindow, vr
	var activeRealityType = 'ar';

	THREE.WebXRUtils.getDisplays().then(init);

	function init(displays) {
		container = document.createElement( 'div' );
		document.body.appendChild( container );
		
		scene = new THREE.Scene();
		camera = new THREE.PerspectiveCamera();
		scene.add( camera );

		renderer = new THREE.WebGLRenderer( { alpha: true } );
		renderer.autoClear = false;
		container.appendChild( renderer.domElement );

		// Add custom content
		reticle = new THREE.Mesh(
		  new THREE.RingGeometry(0.04, 0.05, 36, 64),
		  new THREE.MeshStandardMaterial({ color: '#336699' })
		);
		reticle.geometry.applyMatrix(new THREE.Matrix4().makeRotationX(THREE.Math.degToRad(-90)));
		reticle.visible = false;
		scene.add(reticle);

		pin = new THREE.Mesh(
		  new THREE.SphereGeometry(0.05, 4, 2),
		  new THREE.MeshStandardMaterial({ color: '#336699', flatShading: true })
		);
		pin.position.y = 0.05;
		reticle.add(pin);

		// Add a few lights
		ambientLight = new THREE.AmbientLight('#f8f8f8', initialAmbientLightIntensity);
		scene.add(ambientLight);
		directionalLight = new THREE.DirectionalLight('#f8f8f8', initialDirectionalLightIntensity);
		directionalLight.position.set(5, 5, 2);
		scene.add(directionalLight);

		// End custom content
		window.addEventListener( 'resize', onWindowResize, false );
		onWindowResize();

		var options = {
		  // Flag to start AR if is the unique display available.
		  AR_AUTOSTART: true
		}

		renderer.xr = new THREE.WebXRManager(options, displays, renderer, camera, scene, update);
		renderer.xr.addEventListener('sessionStarted', sessionStarted);

	}

	function sessionStarted(data) {
		activeRealityType = data.session.realityType;
	}

	function onWindowResize() {
		setTimeout(() => {
			camera.aspect = window.innerWidth / window.innerHeight;
			camera.updateProjectionMatrix();
		}, 150);
	}

	// Called once per frame, before render, to give the app a chance to update this.scene
	function update(frame) {
		var hit = frame.hitTestNoAnchor(0.5, 0.5);

		var model = new THREE.Matrix4();
		var tempPos = new THREE.Vector3();
		var tempQuat = new THREE.Quaternion();
		var tempScale = new THREE.Vector3();
		if (hit && hit.length > 0) {
			reticle.visible = true
			model.fromArray(hit[0].modelMatrix);
			model.decompose(tempPos, tempQuat, tempScale);
			reticle.position.copy(tempPos);
			reticle.quaternion.copy(tempQuat);
		}
		if(frame.hasLightEstimate){
		  ambientLight.intensity = frame.lightEstimate * initialAmbientLightIntensity;
		  directionalLight.intensity = frame.lightEstimate * initialDirectionalLightIntensity;
		}
		pin.rotation.y += 0.01;
	}
}